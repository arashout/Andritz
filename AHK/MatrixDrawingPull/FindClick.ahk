FindClick(ImageFile="", Options="", ByRef FoundX="", ByRef FoundY="") ; updated July 23, 2016 ... https://autohotkey.com/boards/viewtopic.php?f=6&t=18719
{
	Static Cache
	...
	;----------------------------------------------------------------------------------------------------
	DefaultDirs = %A_ScriptDir%|%A_Desktop%|%A_MyDocuments%                                                 ; Pipe (|) delimited list of default directories to search if file not found in working directory
	...
	LastUpdate = July 23, 2016                                                                              ; Date of last update
	;||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||
		...
		Loop {
			...
			If (ErrorLevel = 2) {
				...
				If (e <> "") {
					...
					Break
			} Else If (w2 = "") or SubRegions or (w1 and (A_TickCount > w1)) { ; XXX
				If DxInfo
	...
	Else If (InStr(ImageFile, ">Debug:") = 1) ;--------------------------------------------------Debugging--------------------------------------------------
	{
		...
		EditorPath = %A_AhkPath%\..\..\Utilities\Notepad++\Notepad++.exe             ; Path to editor utilized when you select "Go To Line" command
		...
		AllowOffset = %True%                     ; True to have this checkbox cleared by default
